...
## 架构概览
- 前端：Vue3 + Vite + TypeScript，组件库可用 Element Plus。提供登录、列表、筛选、详情时间线、CSV 导入/删除、平台 API 配置操作。
...
## 模块拆解
1. **鉴权模块**
   - 功能：用户名密码登录、Token 下发与刷新、登出、前端路由守卫；登录失败计数与基础防刷。
   - 表：可在 `users` 表（MVP 可内置账号配置）存储账号信息；Token 使用 Redis 黑名单/TTL 控制。
2. **店铺模块**
...
8. **平台 API 配置模块**
   - 功能：按店铺维护 17TRACK 与平台 API Key/Secret/Access Token；支持“测试连接”接口调用；凭证字段脱敏展示；更新后触发重新同步/刷新 Token。
   - 表：`shops` 中存储凭证字段；可扩展 `shop_credentials_history` 记录变更。

## 前端页面
- 登录页：输入账号密码，登录后跳转到运单列表；支持记住会话、登出。
- 店铺管理页：列表、创建/编辑弹窗。
- 平台 API 配置页/抽屉：在店铺详情或单独页面维护 17TRACK 与平台凭证，支持测试连接、脱敏显示。
